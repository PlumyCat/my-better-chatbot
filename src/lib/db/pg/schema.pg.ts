...
export const ArchiveSchema = pgTable("archive", {
  ...
  userId: uuid("user_id")
    .notNull()
    .references(() => UserSchema.id, { onDelete: "cascade" }),
  // MCP integration columns
  mcpMetadata: json("mcp_metadata"),
  mcpServerId: uuid("mcp_server_id").references(() => McpServerSchema.id, { onDelete: "set null" }),
...
export const ArchiveItemSchema = pgTable(
  "archive_item",
  {
    ...
    userId: uuid("user_id")
      .notNull()
      .references(() => UserSchema.id, { onDelete: "cascade" }),
    // MCP analysis columns
    mcpAnalysis: json("mcp_analysis"),
    tokensCount: bigint("tokens_count", { mode: "number" }),
...
// File Attachments table - NEW, doesn't modify existing schema
export const FileAttachmentSchema = pgTable("file_attachment", {
  id: uuid("id").primaryKey().notNull().defaultRandom(),
  ...
  uploadedBy: uuid("uploaded_by")
    .notNull()
    .references(() => UserSchema.id), // reference existing user
  messageId: text("message_id"), // optional reference to ChatMessageSchema.id
  createdAt: timestamp("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  updatedAt: timestamp("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
  index("file_attachment_uploaded_by_idx").on(table.uploadedBy),
  index("file_attachment_content_hash_idx").on(table.contentHash),
