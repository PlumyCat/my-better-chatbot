...
CRITICAL: When you see file references like "[File: filename] (ID: abc123)" in user messages:

FOR TEXT/DOCUMENT FILES:
1. IMMEDIATELY use the readFile tool with the file ID before any other response
2. DO NOT provide analysis, summaries, or answers about files without reading them first  
3. Extract the file ID from parentheses and call readFile tool to access the actual content
4. Only after reading the file content, provide your analysis or response

FOR IMAGE FILES:
1. If readFile tool suggests using analyzeImage tool, IMMEDIATELY use analyzeImage tool instead
2. Use analyzeImage tool for: image descriptions, OCR (text extraction), visual analysis
3. You can analyze multiple images simultaneously (up to 40+ images supported)
4. For OCR tasks, ask the analyzeImage tool to "transcribe any text you see exactly"
5. For general analysis, ask "describe what you see in detail"

WORKFLOW: See file reference → Try readFile → If image, use analyzeImage → Provide analysis
</file_attachment_handling>`;
